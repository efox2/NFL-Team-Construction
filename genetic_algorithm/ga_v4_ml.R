...
ml_rpart = readRDS("./rpart_model.rds")
...
evalFunc <- function(x) {
  ...
  x <- c('Quarter.Back_1','Quarter.Back_2','Quarter.Back_3','Running.Back_1','Running.Back_2','Running.Back_3','Running.Back_4','Offensive.Lineman_1','Offensive.Lineman_2',
  ...
  colnames(rpartdf) <- x
  print(list(ml_input))
  cat(paste(shQuote(ml_input, type="cmd"), collapse=", "))
  de <- data.frame(ml_input)
  print(de)
  rpartdf[nrow(rpartdf) + 1,] <- rbind(as.matrix(rpartdf),as.matrix((de)))
  print(rpartdf)
  # ml_input is the input to the machine learning function
  # ml_output is the output of the machine learning function
  #####
  
  ml_input_rpart=ml_input
